...
import BrushController, {
  BrushControllerEvents,
  BrushDimensionMinMax
} from "../../helper/BrushController";
import BrushTargetManager, {
  BrushTargetInfoCartesian2D
} from "../../helper/BrushTargetManager";
...
class DataZoomFeature extends ToolboxFeature<ToolboxDataZoomFeatureOption> {
  ...
  private _onBrush(eventParam: BrushControllerEvents["brush"]): void {
    ...
    brushTargetManager.matchOutputRanges(areas, ecModel, function(
      area,
      coordRange,
    ...
    function setBatch(
      ...
      // Restrict range.
      const minMaxSpan = dataZoomModel
...
function makeAxisFinder(
  dzFeatureModel: ToolboxDataZoomFeatureModel
): ModelFinderObject {
  const setting = {
...
function updateZoomBtnStatus(
  ...
  const panels = brushTargetManager.makePanelOpts(api, function(
    targetInfo: BrushTargetInfoCartesian2D
  ) {
  ...
  view._brushController.setPanels(panels).enableBrush(
...
registerInternalOptionCreator("dataZoom", function(
  ecModel: GlobalModel
): ComponentOption[] {
  ...
  const dzFeatureModel = toolboxModel.getModel(
    featureDataZoomPath as any
  ) as ToolboxDataZoomFeatureModel;
  ...
  each(finderResult.xAxisModels, axisModel =>
    buildInternalOptions(axisModel, "xAxis", "xAxisIndex")
  );
  each(finderResult.yAxisModels, axisModel =>
    buildInternalOptions(axisModel, "yAxis", "yAxisIndex")
  );
