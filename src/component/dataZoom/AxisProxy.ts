...
class AxisProxy {
      ...
      if (filterMode === "weakFilter") {
        ...
        seriesData.filterSelf(function(dataIndex) {
          ...
          for (let i = 0; i < dataDims.length; i++) {
            ...
            const thisLeftOut = value < valueWindow[0] - 1;
            const thisRightOut = value > valueWindow[1] + 1;
  ...
  private _updateMinMaxSpan() {
    ...
    each(
      ["min", "max"],
      function(minMax) {
        let percentSpan = dataZoomModel.get((minMax + "Span") as
          | "minSpan"
          | "maxSpan");
        let valueSpan = dataZoomModel.get((minMax + "ValueSpan") as
          | "minValueSpan"
          | "maxValueSpan");
        valueSpan != null &&
